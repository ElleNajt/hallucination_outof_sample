...
def plot_accuracy_by_confidence(
    ...
    finite_data = [(c, corr) for c, corr in zip(confidence_ratios, correct) if c != float('inf')]
    ...
    for i in range(1, num_bins + 1):
        ...
        bin_results = [finite_correct[j] for j in range(len(finite_correct)) if bin_mask[j]]
