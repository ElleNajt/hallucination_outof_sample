...
def analyze_results(log_file):
    ...
    all_scores = [(s, 'valid') for s in valid_scores] + [(s, 'invalid') for s in invalid_scores]
    ...
    print(f"Total correct: {valid_correct + invalid_correct}/{valid_total + invalid_total}")
...
def plot_results(stats, output_file):
    ...
    ax = axes[0, 0]
    ax.hist(stats['valid_scores'], bins=30, alpha=0.6, label='Valid syllogisms', color='green')
    ax.hist(stats['invalid_scores'], bins=30, alpha=0.6, label='Invalid syllogisms', color='red')
    ax.axvline(stats['best_threshold'], color='black', linestyle='--', linewidth=2, label=f'Threshold: {stats["best_threshold"]:.4f}')
    ...
    ax.axhline(stats['best_threshold'], color='black', linestyle='--', linewidth=2, label=f'Threshold: {stats["best_threshold"]:.4f}')
...
def main():
    parser = argparse.ArgumentParser(description='Analyze syllogistic reasoning results')
    parser.add_argument('log_file', type=str, help='Path to log file')
    parser.add_argument('--output', type=str, help='Output plot file (default: same as log file with _plot.png)')
    ...
    if args.output:
        ...
        output_path = log_path.with_suffix('').with_suffix('.png').with_name(log_path.stem + '_plot.png')
